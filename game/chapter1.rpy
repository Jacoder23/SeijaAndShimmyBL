...
label st_chapter_start_1:
    ...
    $ party_one = [{"name":"Wishmaker",
                    ...
                    "effects":[],
                    "options":[[["selecting_target = True", "violence += 1; dmg_to_target = 3; noglobal QueueSFX('PUNCH_PERCUSSIVE_HEAVY_09.opus')", "noglobal QueueSFX('WHOOSH_ARM_SWING_01.opus'); violence += 0.5", 10, (functools.partial(fstr, "Power{', Observed Backswitch' if precision > 0 else ''}"), shin_battler.power),               # run prior to outcome, effect on success, effect on failure, DC (1d20), relevant stat (if any)
